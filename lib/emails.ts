...
const baseStyle = `font-family:Georgia,serif;background:#0f0f0f;color:#fafaf8;padding:40px;max-width:500px;margin:0 auto;border-radius:8px;`
  ...
  const html = `
    <div style="${baseStyle}">
      ...
      <div style="margin-top:24px;padding:16px;background:#1a1a1a;border-radius:6px;border-left:3px solid #c9a84c;">
        <p style="margin:0 0 12px;font-size:13px;color:#aaa;">¿Necesitás cancelar? Podés hacerlo desde el link de abajo con al menos 2 horas de anticipación.</p>
        <a href="${APP_URL}/mis-turnos" style="display:inline-block;padding:8px 16px;background:#c9a84c;color:#0f0f0f;text-decoration:none;border-radius:5px;font-size:13px;font-weight:bold;">Ver / cancelar mi turno →</a>
      </div>
...
export async function sendCancellationEmail(
  ...
  const html = `
    <div style="${baseStyle}">
      ...
      <p>Hola <strong>${safeFirstName}</strong>, tu turno del <strong>${dateStr} a las ${safeTime}</strong> fue cancelado.</p>
      <p style="color:#888;font-size:14px;">Podés reservar un nuevo turno desde nuestra web cuando quieras.</p>
